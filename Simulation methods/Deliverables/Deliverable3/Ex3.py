...
def apply_RK(rgk45_lib, x_0, ode_ptr = ode_ptr, t_0 = 0.0, h_p=0.01, atf_p=0.1, tol_p=1e-6, aer_p =0.0):
    ...
    for i in range(int(Niters)):
        value= rgk45_lib.rkf45(byref(at), x.ctypes.data_as(POINTER(c_double)), n, byref(h), sc, tol, byref(atf), byref(aer), ode_ptr)     
...
def Newton_method(rgk45_lib, ode_ptr = ode_ptr, x_0=[0,0.01], t_0 = 0, v_0 = [1, 0, 0, 1], Nmax= 100, tol= 1e-4, h__ = 2*np.pi):
    ...
    while niter<Nmax and tol_x>tol:
        ...
        print('Input x_0:', x_0)        
        x_aux, t_aux = Newt_step(rgk45_lib=rgk45_lib, ode_ptr = ode_ptr, x_0 = x_0, t_0 = t_0, v_0= v_0, h__ = h__)
...
def Newt_step(rgk45_lib, ode_ptr= ode_ptr, x_0=[0,0.01], t_0 = 0, v_0 = [1, 0, 0, 1], h__ = 2*np.pi):   
    xx = np.concatenate((x_0, v_0), axis = None)   
    t_1, xx_1 = apply_RK(rgk45_lib, ode_ptr=ode_ptr, x_0= xx, t_0 = t_0, h_p=0.01, atf_p= t_0 + h__, tol_p=1e-6, aer_p =0.0)       
    x_1 = xx_1[::6]
    y_1 = xx_1[1::6]
    v_11 = xx_1[2::6]
    v_12 = xx_1[3::6]
...
def generate_orbits(rgk45_lib, x_0, t_0 = 0.0, atf_p=2*np.pi, NP = 20):
...
vv = np.concatenate((x_0, v_0), axis = None)
print(vv.shape)
print(vv)
print('---------')
print('Trying one step:')
# ode_ptr= ode_ptr(w = np.sqrt(2)),
x_1, t_1 = Newt_step(rgk45_lib=rgk45_lib, x_0 = x_0, t_0 = t_0, v_0= v_0, h__= h__)
print(x_1, t_1)
print('Tolerance first step:', np.linalg.norm( x_1[0] - x_0[0]))

#value = apply_RK(rgk45_lib, vv, ode_ptr = ode_ptr, t_0 = 0.0, h_p=0.01, atf_p=2*np.pi, tol_p=1e-6, aer_p =0.0) 
#print(vv)
#print(vv.shape)
#print(value[0].shape)
#print(value[1].shape)
#print(value[1].shape/6)

"""   
print('---------')
print('Trying 15 steps:')
x_2, t_2 = Newton_method(rgk45_lib, x_0=x_0, t_0 = t_0, v_0 = v_0, Nmax= NP, tol= 1e-4, h__ = h__)
print(x_2, t_2)
xx_1 = x_2[::6]
yy_1 = x_2[1::6]
print(len(xx_1))
...
xx_ = generate_orbits(rgk45_lib, x_0, t_0 = 0.0, atf_p=2*np.pi, NP = 20)
plot_orbit(xx_[::2], xx_[1::2], picture_path = './Ex1.png')
