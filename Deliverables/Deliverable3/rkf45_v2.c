...
//static inline void odefunc(double t, double* y, int n, double* dydt);
...
ode: pointer to the vectorfield. The parameters of the function
...
static inline double step_rk(double *x, int n, double k[6][n])
/*
Computation of the actual RK step depending on the fixed parameter RK
*/
{
  double nr;
  int i;
...
static inline void compute_ks(double t, double *x, int n, double h,
              double k[6][n], void (*ode)(double,double*,int,double*))
/*
computation of the intermediate k_s
used in the Runge-Kutta-Fehlberg algorithm for a given 
time t, initial condition x, step size h, and function 
ode that defines the differential equation.
*/

/*
(*ode)(double,double*,int,double*)
has as parameters:
t
*x: pointer to initial points x,y
int n: dimension of ode
double* k[0] the value of the vectorifield f(t, x)
*/

{
  ...
  // Each time we call the ode we update the value of the pointer k*
  (*ode)(t,x,n,k[0]);
  for (i=0; i<n; i++)
  {
    k[0][i] *= h;
...
// Definition of the given ODE
void odefunc(double t, double* y, int n, double* dydt) {
    double x = y[0];
    double v = y[1];
    double omega = sqrt(2); 
    double epsilon = 0.01; 
    n = sizeof(y)/sizeof(double);

    dydt[0] = v;
    dydt[1] = -omega*omega*sin(x) + epsilon*sin(t);
}
